...
export function bridgeMainFactory(target: Window): void {
  ...
  target.addEventListener('message', (ev: MessageEvent) => {
    ...
    // 只处理响应包（带 kind 字段）；请求包（带 method）由 relay 转发，本桥接层不消费
  ...
  const api = {
    chat: { completions: {
      create: (params: { model: string; messages: Array<{ role: string; content: string; [k: string]: unknown }>; stream?: boolean; tools?: unknown[]; tool_choice?: unknown; apiKey?: string; conversation_id?: string }): Promise<ChatCompletion> | (AsyncIterable<ChatCompletionChunk> & { cancel(): Promise<void> }) => {
        const merged = { ...(target.deepApiConfig ?? {}), ...params };
        const id = send('chat.completions.create', merged);
        if (merged.stream === true) return streamHandle(id);
