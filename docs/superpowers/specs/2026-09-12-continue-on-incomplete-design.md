...
**日期**：2026-09-12
**状态**：草案，待 user review
**触发**：DeepSeek 网页生成中途断流时 UI 显示 Continue 按钮，需用户手动点击才能继续；deep.api 应自动续接，客户端无感
...
1. deep.api 自动调 `POST /api/v0/chat/continue` 续接，把续接内容**无缝并入同一条流**（无重复、无缺口、客户端无感）；
2. 续接成功后正常 `finish_reason` 收尾（工具调用等后处理与正常路径完全一致）；
3. 续接仍失败（服务端持续故障）→ 达到上限后维持 v0.2.2 行为报 503。

**成功标准**：客户端视角与「模型一口气说完」不可区分。
...
4. `run.continueAttempts < MAX_CONTINUE_ATTEMPTS`（默认 3，含首次断流后的第 1 次续接）。
...
- headers：`baseHeaders` + `x-client-*` 指纹（与 completion 相同，**不调 PoW**）——抓包同款；
- 响应：`fetchStreamSafe` 校验 200 后，`completionEvents(res.body, NO_PROGRESS_MS, onReady, skip)`（F4）。
...
- router 按**原始增量**累计已发出字符数（`run.emittedThinkChars` / `run.emittedContentChars`，在 `consumeEvent` 与 `encodeStream` 两路、在任何归一化之前的 `think_delta`/`content_delta` 上累加）；续接时把这两个数作为 `skip` 传入；
- parser 的 `makeProcessor` 接收 skip 参数，**只裁剪来自快照（`applySnapshot`）的事件**头部字符：
  ...
  - skip 归零后不再裁剪；
- **不裁剪追加增量（appends）**：若某次续接流不带快照（未观测），appends 即新内容，直接放行，无吞字风险；
...
- **不走队列锁**：续接发生在 provider 流已耗尽之后，直调 `provider.continueStream`（与 `repairToolCalls` 直调 `streamCompletion` 同理——再 acquire 会自锁 ≤60s 超时成 429）；
...
| 文件 | 改动 |
|---|---|
| `src/background/providers/deepseek/sse-patch.ts` | `completionEvents(body, timeoutMs, onReady, skip?)`；`makeProcessor` 持有 skip 剩余量，快照分支裁剪后 emit |
| `src/background/providers/deepseek/client.ts` | 新增 `continuePayload(session, messageId)` → `{chat_session_id, message_id, fallback_to_resume: true}`；新增 `continueHeaders(token)`（`baseHeaders` + x-client 指纹，无 PoW）——`withPowHeaders` 改为复用它 |
| `src/background/providers/deepseek/adapter.ts` | 新增 `continueStream(ctx, session, messageId, skip)`（`baseHeaders`+x-client 头，无 PoW；复用 `completionEvents`） |
| `src/background/providers/adapter.ts` | `ProviderAdapter` 接口 + `continueStream` 签名 |
| `src/background/router.ts` | `RunState` 加 `continueAttempts` / `emittedThinkChars` / `emittedContentChars`；两路的事件源包「续接 wrapper」；流末 503 检查移到续接循环之后；新增常量 `MAX_CONTINUE_ATTEMPTS=3` / `CONTINUE_DELAY_MS=500` / `RESUMABLE_REASONS` |
| `src/background/log.ts` | `LogEntry.continueAttempts?: number`（诊断：成功续接了几次） |
| 测试 | 见 §4 |
...
- `stream_stats`（sseBytes/paths/statusValues…）为**最后一段**的值（续接成功时即成功段）——足够定位「第几段坏、坏在哪」，不聚合；
...
4. 回归：skip=0（正常 completion）行为与现状完全一致。

**单元（client）**：`continuePayload` 字段形状 + 无 PoW 头组合（对照抓包）。

**集成（router + stub adapter）**：

5. fail-to-pass：completion 段 stream_error → continueStream 返回 FINISHED 段 → 单次成功响应，聚合内容 = 首段 + 续段（无重复，skip 生效）；
6. 连续两次断流 → 两次续接成功（attempts=2）；
7. 三次续接仍 stream_error → 503（上限收敛）；
8. `unsupported_client_by_model` → **不调用** continueStream，直接 503；
9. stream:true 路径同 5（分块连续、无重复、正常 finish chunk）；
10. 回归：正常流零续接（continueStream 不被调用）。
...
| 续接期间服务端把消息标为不可续（fallback 新建 message） | ready 的 `response_message_id` 会更新 `run.parentMessageId`，后续段与 commit 自然跟随（F6 设计已覆盖） |
| 队列/并发：续接直调绕队列 | 与 repairToolCalls 同例：流已耗尽，锁已释放，直调安全；无并发新增 |
